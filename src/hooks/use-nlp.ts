...
import {
  getAllFormats,
  getVersionByFormat,
  getTransactionSetByVersion,
  getSegmentByTransactionSet,
  getElementBySegment,
  setElementBySegment,
} from "../actions/nlp";
...
export const useGetTransactionSetByVersion = (
  version: string,
  agency: string
) => {
  return useQuery({
...
export const useGetSegmentByTransactionSet = (
...
export const useGetElementBySegment = (
  segment: string,
  version: string,
  agency: string
) => {
  ...
  return useQuery({
    ...
    queryFn: () =>
      getElementBySegment(segment, version, agency, tenant_id ?? ""),
    enabled: !!segment && !!agency,
  });
};

export const useSetElementBySegment = (
  segment: string,
  version: string,
  agency: string
) => {
  ...
  return useMutation({
    mutationFn: (elements: any) =>
      setElementBySegment(segment, version, agency, tenant_id ?? "", elements),
    onSuccess: () => {
