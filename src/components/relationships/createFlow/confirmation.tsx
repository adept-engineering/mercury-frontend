...
import { CheckCircle, FileText, Settings, ChevronDown, ChevronRight, XCircle, ShieldCheck, Shuffle, Search } from 'lucide-react';
...
export function Confirmation({
  ...
  endPointUrl,
    ...
    const getTypeIcon = (mapType: string) => {
        switch (mapType) {
            case "COMPLIANCE":
        ...
        <div className="space-y-6">
            ...
            {/* Relationship Name */}
              ...
              <div className='flex w-full bg-card text-card-foreground rounded-xl p-4 border shadow-sm justify-between gap-2'>
                ...
                <p className="text-muted-foreground">
                    {relationshipName || <span className="text-muted-foreground italic">Not specified</span>}
                </p>
              </div>

              <div className='flex w-full bg-card text-card-foreground rounded-xl p-4 border shadow-sm justify-between gap-2'>
                <h2 className="text-lg font-semibold text-foreground flex items-center gap-2"><Settings className="w-5 h-5" />Endpoint URL</h2>
                <p className="text-muted-foreground">
                    {endPointUrl || <span className="text-muted-foreground italic">Not specified</span>}
                </p>
              </div>
                        ...
                        <CardContent className="space-y-4">
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <p className="mt-1">{selectedSenderEntity ? getEntityName(selectedSenderEntity) : 'Not set'}</p>
                                </div>
                                <div>
                                    <label className="text-sm font-medium text-muted-foreground">Sender Reference</label>
                                ...
                                {selectedTransactionSet && (
                                    <div>
                                        <label className="text-sm font-medium text-muted-foreground">Transaction Set</label>
                        ...
                        <CardContent className="space-y-4">
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <p className="mt-1">{selectedReceiverEntity ? getEntityName(selectedReceiverEntity) : 'Not set'}</p>
                                </div>
                                <div>
                                    <label className="text-sm font-medium text-muted-foreground">Receiver Reference</label>
                                    <p className="mt-1">{selectedReceiverReference || 'Not set'}</p>
                                </div>
                            </div>
                        </CardContent>
                    </CollapsibleContent>
            ...
            {/* Business Rules */}
            <Collapsible open={openSections.businessRules} onOpenChange={() => toggleSection('businessRules')}>
                <Card className=' px-4'>
                    <CollapsibleTrigger className="w-full ">
                        <CardHeader className=' pt-6 border-b'>
                            <CardTitle className="flex items-center justify-between">
                                ...
                                <div className="space-y-3">
                                    {businessRules.map((rule, index) => (
                                        <div key={rule.id} className="flex items-center gap-3 p-3 border rounded-lg">
                                            <div className="flex items-center gap-2">
                                                {getTypeIcon(rule.map_type)}
                                                <div>
                                                    <p className="font-medium">{getTypeLabel(rule.map_type)}</p>
