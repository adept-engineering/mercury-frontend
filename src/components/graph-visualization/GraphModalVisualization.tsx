...
const GraphModalVisualization: React.FC<GraphModalVisualizationProps> = ({ data }) => {
  ...
  // Debug logging
  console.log("GraphModalVisualization data:", data);
  ...
  const handleResetZoom = () => {
    try {
      if (nvlRef.current) {
        if (nvlRef.current.zoomToNodes && data.nodes.length > 0) {
          const nodeIds = data.nodes.map((n) => n.id);
          nvlRef.current.zoomToNodes(nodeIds);
  ...
  const styledNodes = data.nodes.map((node: any) => {
    ...
    if (node.labels.includes("Client")) {
      if (node.properties.is_main_sender) {
        ...
        size = 30;
        label = node.properties.name || "Main Sender";
      } else if (node.properties.is_main_receiver) {
        color = "#4ecdc4";
        size = 30;
        label = node.properties.name || "Main Receiver";
      } else {
        color = "#4ecdc4";
        size = 20;
  ...
  return (
    <div className="flex h-full w-full">
      {/* Left sidebar with summary and legend */}
          ...
          <div className="mb-6">
            ...
            <div className="space-y-2 text-sm">
              <div className="flex justify-between">
                ...
                <span className="font-medium">{data.summary.total_nodes}</span>
              </div>
              <div className="flex justify-between">
                <span>Total Relationships:</span>
                <span className="font-medium">{data.summary.total_relationships}</span>
              </div>
              <div className="flex justify-between">
                <span>Client Nodes:</span>
                <span className="font-medium">{data.summary.client_nodes}</span>
              </div>
              <div className="flex justify-between">
                <span>Transaction Nodes:</span>
                <span className="font-medium">{data.summary.transaction_nodes}</span>
              </div>
              <div className="flex justify-between">
                <span>Connected Clients:</span>
                <span className="font-medium">{data.summary.connected_clients ?? (data.summary as any)?.involved_clients ?? "-"}</span>
              </div>
            </div>
          </div>
        )}
