...
export function DataFormat() {
  ...
  const { data: transactionSets, isLoading: isLoadingTransactionSets } =
    useGetTransactionSetByVersion(selectedVersion || "", selectedFormat || "");
  ...
  const { mutate: setElementBySegment } = useSetElementBySegment(
    selectedSegment || "",
    selectedVersion || "",
    selectedFormat || ""
  );
  ...
  const parsedTemplate = parseTemplateToArray(
    elements !== undefined ? elements[0].description : ""
  );
