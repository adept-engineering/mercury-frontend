...
export const createMap = async (token: string, data:{map_name:string, map_description:string, map_type:string, maps_rules:{rule:string, rule_title:string,position:number,}[]}) => {
    try{
...
export const deleteMap = async (token: string, mapId: string, maps_id: string) => {
    try{
        const response = await axiosInstance(token).delete(`/maps?mapId=${mapId}&maps_id=${maps_id}`)
        revalidatePath("/maps")
        return response.data
    }catch(error){
...
export const createMapRules = async (token: string, mapId: string,data:{map_id:string,ruleTitle:string,rule:string,position:number}) => {
    try{
...
export const editMapRules = async (token: string, mapId: string, data:{ruleTitle:string,rule:string,position:number,id:string}) => {
    try{
...
export const deleteMapRules = async (token: string, mapId: string, id:string) => {
    try{
